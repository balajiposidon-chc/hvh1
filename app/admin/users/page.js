...
export default async function AdminUsersPage() {
    ...
    return (<AdminLayout>
      ...
      <AdminUserTable users={users.map((user) => ({ id: user._id.toString(), name: user.name, email: user.email, phone: user.phone || 'N/A', role: user.role, status: user.status, createdAt: user.createdAt.toISOString() }))} mainAdminEmail="admin@store.com"/>
    </AdminLayout>);
}
